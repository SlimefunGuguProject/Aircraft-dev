...
public class Utils {
    ...
    public Vector3d rotate(final @NotNull Vector3d vector, final @NotNull Vector3d rotation) {
        return new Vector3d(vector).rotateZ((float) rotation.z).rotateY((float) rotation.y).rotateX((float) rotation.x);  // new Vector3d(vector).rotateAxis(rotation.length(), rotation.x, rotation.y, rotation.z);
    }

    public Vector3f rotate(final @NotNull Vector3f vector, final @NotNull Vector3d rotation) {
        return new Vector3f(vector).rotateZ((float) rotation.z).rotateY((float) rotation.y).rotateX((float) rotation.x); //new Vector3f(vector).rotateAxis((float) rotation.length(), (float) rotation.x, (float) rotation.y, (float) rotation.z);
    }

    public Matrix4f rotate(final @NotNull Vector3d rotation) {
        return new Matrix4f().rotateZYX(new Vector3f((float) rotation.z, (float) rotation.y, (float) rotation.x)); // new Matrix4f().rotate((float) rotation.length(), (float) rotation.x, (float) rotation.y, (float) rotation.z);
    }
}
